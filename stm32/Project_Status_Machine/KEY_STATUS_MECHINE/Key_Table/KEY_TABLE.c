...
void Key_Scan_Engine(buttonType *btn) {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
    ...
    if (button_state_table[i].currentState == btn->keyAction &&
        button_state_table[i].triggerEvent == currentEvent) {
      // 1. 执行伴随动作
...
void Action_NullTick(buttonType *btn) {
  ...
  if (btn->hold_cnt > LONG_PRESS_TIME) {
    ...
    Key_ParaInit(btn);
  } else if (btn->hold_cnt < LONG_PRESS_TIME && btn->hold_cnt > CLICK_MAX_TIME) {
    Key_ParaInit(btn); // 无效长按，直接清零
